...
好的提示：→ “请解释 REST API 如何处理身份验证问题。请给出一个包含代码的实际示例。假设我是一名初级开发者。”→ 结果：具体、有条理，且立即有用。
...
真正有效的技巧包括：提供上下文（“我正在为 X 项目开发 SaaS 服务”）;分配角色（“担任高级后端工程师”）;举例说明（“这里有个我喜欢的格式：___”）;明确说明输出结果（“请列出 5 个选项，并用编号列表表示”）;将复杂的任务分解为简单的步骤。
